...
class TTSRLModel(pl.LightningModule):
    # Mel time-axis factor before HiFi-GAN (16 kHz vs 22.05 kHz frame alignment); inverted on
    # the waveform before HuBERT. All vocoder inputs must go through _rescale_mel_time_for_vocoder.
    vocoder_mel_time_scale: float = 22050 / 16000
    ...
    def __init__(
        ...
        rl_temperature: float =0.05,
        rl_entropy_coef: float =0.001,
    ...
    def _save_audio_if_needed(
        ...
        for i in range(max_to_save):
            ...
            sf.write(
                str(out_dir / f"epoch_{self.current_epoch}_step_{batch_idx}_sample_{i}.wav"),
                w,
                22050,
            )
    ...
    def _waveform_for_hubert(self, wav: Tensor) -> Tensor:
        """
        Invert ``_rescale_mel_time_for_vocoder`` on the waveform: undo mel time stretch
        before HuBERT (same 22.05 kHz sample rate as vocoder output).
        """
        s = float(self.vocoder_mel_time_scale)
        if wav.dim() == 3:
            wav = wav.squeeze(1)
    ...
    def _wav_samples_from_mel_frames(self, n_mel_frames: Tensor, s: float, seq_len_wav: int) -> Tensor:
        """Mel frame counts → 22.05 kHz vocoder-output sample lengths (before time-undo)."""
    ...
    def training_step(self, batch, batch_idx: int) -> Tensor:
        ...
        reward = (1-abs_diff) - tts_loss.detach() #torch.exp(-abs_diff / 0.1)
